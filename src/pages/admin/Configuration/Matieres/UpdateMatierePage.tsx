...
export default function UpdateMatierePage({}: PagesProps) {
  ...
  const handleSubmit = async (dataArray: MatiereFormData[]) => {
    ...
    try {
      ...
      setSaved(true);
      alertSuccess("Matière modifiée avec succès");
      setTimeout(() => {
        navigate(-1);
      }, 1500);
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] animate-fade-in-up">
        ...
        <p className="text-gray-500 font-medium">Chargement de la matière...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-2xl mx-auto mt-10 animate-fade-in-up">
        <div className="bg-gradient-to-r from-red-50 to-red-100 border border-red-200 rounded-2xl p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-red-800 mb-2">Matière introuvable</h2>
          <p className="text-red-600 mb-6">Les informations de cette matière ne sont plus disponibles.</p>
          <button
            onClick={() => navigate(-1)}
    ...
    <div className="space-y-6 pb-8">
      ...
      {saved && (
        <div className="bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-xl p-4 flex items-center gap-3 animate-fade-in">
          ...
          <span className="text-green-700 font-medium">Matière modifiée avec succès ! Redirection...</span>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-4 justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-medium text-gray-800">{matiere.niveauClasse}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white rounded-xl shadow-sm">
            ...
            <div>
              ...
              <p className="font-medium text-gray-800">{matiere.coefficient}</p>
            </div>
          </div>
        </div>
      </div>
