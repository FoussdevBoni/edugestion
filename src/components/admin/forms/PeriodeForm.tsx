// src/components/forms/PeriodeForm.tsx
...
export default function PeriodeForm({ 
  ...
  const validate = (): boolean => {
    ...
    if (!formData.nom.trim()) {
      newErrors.nom = "Le nom de la période est requis";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      // Ajouter le nom du niveau scolaire
  ...
  const handleChange = (field: keyof PeriodeFormData, value: string) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: 1er Trimestre, 1er Semestre, etc."
            />
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        >
          {isSubmitting ? "Enregistrement..." : initialData ? "Mettre à jour" : "Créer la période"}
        </button>
      </div>
    </form>
  );
